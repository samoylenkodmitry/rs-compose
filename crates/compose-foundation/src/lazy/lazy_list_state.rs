...
impl LazyListScrollPosition {
    ...
    pub(crate) fn update_from_measure_result(
        ...
        if self.index.get() != first_visible_index {
            self.index.set(first_visible_index);
        }
        if (self.scroll_offset.get() - first_visible_scroll_offset).abs() > 0.001 {
    ...
    pub(crate) fn update_if_first_item_moved<F>(&self, new_item_count: usize, find_by_key: F) -> usize
    where
        F: Fn(u64) -> Option<usize>,
    {
        ...
        let new_index = match last_key {
            ...
            Some(key) => find_by_key(key).unwrap_or_else(|| {
                current_index.min(new_item_count.saturating_sub(1))
            }),
        };
    ...
    pub fn nearest_range(&self) -> std::ops::Range<usize> {
        self.inner.with(|rc| rc.borrow().nearest_range_state.range())
    }
}
...
impl LazyListState {
    ...
    pub fn update_stats(&self, items_in_use: usize, items_in_pool: usize) {
        ...
        if current.items_in_use != items_in_use || current.items_in_pool != items_in_pool {
    ...
    pub fn dispatch_scroll_delta(&self, delta: f32) -> f32 {
        self.inner.with(|rc| {
            rc.borrow_mut().scroll_to_be_consumed += delta;
        });
        self.invalidate();
    ...
    pub(crate) fn consume_scroll_to_index(&self) -> Option<(usize, f32)> {
        self.inner.with(|rc| rc.borrow_mut().pending_scroll_to_index.take())
    }
    ...
    pub fn get_cached_size(&self, index: usize) -> Option<f32> {
        self.inner.with(|rc| rc.borrow().item_size_cache.get(&index).copied())
    }
    ...
    #[deprecated(note = "Nearest range is automatically updated by scroll_position. This is now a no-op.")]
    ...
    pub fn remove_invalidate_callback(&self, id: u64) {
        self.inner.with(|rc| {
            rc.borrow_mut().invalidate_callbacks.retain(|(cb_id, _)| *cb_id != id);
        });
    }

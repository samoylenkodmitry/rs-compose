...
impl LazyLayoutKey {
    ...
    ///
    /// # ⚠️ User Key Limitation
    /// User-provided keys are **masked to 62 bits**. Keys that differ only in
    /// bits 62-63 will produce the **same slot ID** and cause incorrect behavior:
    /// - `User(0x0000_0000_0000_0001)` and `User(0x4000_0000_0000_0001)` collide!
    ///
    /// **Best practice**: Use keys that fit in 62 bits (values < 2^62).
    /// Database IDs, array indices, and hash values typically satisfy this.
    ///
    ...
    pub fn to_slot_id(self) -> u64 {
        match self {
            // NOTE: User keys are masked to 62 bits. See doc comment above.
            LazyLayoutKey::User(k) => {
                // Debug assertion: warn if high bits will be lost
                debug_assert!(
                    k <= Self::VALUE_MASK,
                    "User key {:#018x} exceeds 62 bits - high bits will be lost, causing potential key collision",
                    k
                );
                Self::USER_TAG | (k & Self::VALUE_MASK)
            }
            LazyLayoutKey::Index(i) => {
                // Debug assertion: warn if index exceeds 62 bits (very unlikely for list indices)
                debug_assert!(
                    (i as u64) <= Self::VALUE_MASK,
                    "Index {} exceeds 62 bits - high bits will be lost",
                    i
                );
                Self::INDEX_TAG | ((i as u64) & Self::VALUE_MASK)
            }
        }
    }
...
impl LazyListIntervalContent {
    ...
    pub fn get_index_by_slot_id(&self, slot_id: u64) -> Option<usize> {
        ...
        if self.total_count <= Self::CACHE_THRESHOLD {
            return (0..self.total_count).find(|&index| self.get_key(index).to_slot_id() == slot_id);
        }
...
mod tests {
    ...
    fn test_slot_id_collision_prevention() {
        ...
        // Any user value maps to the user range (masked to 62 bits)
        let user_max = LazyLayoutKey::User(u64::MAX);
        assert!(user_max.to_slot_id() < (1u64 << 62), "User keys stay in user range");
        assert_eq!(user_max.to_slot_id(), (1u64 << 62) - 1); // All 62 value bits set

        // Any index value maps to the index range
        let index_large = LazyLayoutKey::Index(usize::MAX);
        assert!(index_large.to_slot_id() >= (1u64 << 62), "Index keys stay in index range");
        assert!(index_large.to_slot_id() < (2u64 << 62), "Index keys below reserved range");

        // ⚠️ DOCUMENTED LIMITATION: Keys differing only in bits 62-63 WILL COLLIDE!
        // This is intentional - see to_slot_id() documentation.
        ...
        assert_eq!(
            key_low.to_slot_id(),
            key_high.to_slot_id(),
            "Keys differing only in bits 62-63 collide (documented limitation)"
        );
    }

    ...
    AppLauncher::new()
        ...
        .with_test_driver(|robot| {
            ...
            if let Some((field_x, field_y, field_w, field_h)) = text_field_pos {
                ...
                // Check focus
                if !compose_ui::has_focused_field() {
                    println!("✗ FAIL: Text field not focused after click");
                    std::process::exit(1);
                }
                println!("  • Field focused: true");
                ...
                if !compose_ui::has_focused_field() {
                    println!("✗ FAIL: Lost focus during drag");
                    std::process::exit(1);
                }
                println!("  • Still focused after drag: true");

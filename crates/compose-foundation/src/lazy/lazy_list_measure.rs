...
{
    // Warn once if reverse_layout is set (not yet implemented)
    if config.reverse_layout {
        use std::sync::atomic::{AtomicBool, Ordering};
        static WARNED: AtomicBool = AtomicBool::new(false);
        if !WARNED.swap(true, Ordering::Relaxed) {
            log::warn!(
                "reverse_layout is not yet implemented - items will be laid out in normal order"
            );
        }
    }
    ...
    let position = resolver.resolve_and_normalize();

    // 3. Measure items (visible + beyond-bounds buffer)
    let mut measurer = ItemMeasurer::new(measure_item, config, items_count, effective_viewport_size);
    let mut visible_items = measurer.measure_all(
        position.first_item_index,
        position.first_item_scroll_offset,
    );
...
mod tests {
    use super::*;
    use super::super::lazy_list_state::test_helpers::{new_lazy_list_state, new_lazy_list_state_with_position, with_test_runtime};

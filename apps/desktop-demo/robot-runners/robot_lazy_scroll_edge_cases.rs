...
fn main() {
    ...
    AppLauncher::new()
        ...
        .with_test_driver(|robot| {
            ...
            let (_, header_y, _, header_h) = find_button_in_semantics(&robot, "Jump 50")
                .expect("Jump button should exist");
            ...
            println!("  Item 1 y={:.1}, spacing from Item 0: {:.1}px", y1, spacing_0_1);
            ...
            println!("  ✓ Spacing is correct: {:.1}px (expected ~{:.1}px)", spacing_0_1, expected_spacing);
            
            // === Test 2: Jump preserves correct positioning with spacing ===
            println!("\n--- Test 2: Large scroll jump preserves spacing ---");
            
            let (bx, by, bw, bh) = find_button_in_semantics(&robot, "Jump 50")
                .expect("Jump button should exist");
            ...
            if let Some((_, y51, _, _)) = item_51 {
                ...
                println!("  Item 51 y={:.1}, spacing from Item 50: {:.1}px", y51, spacing_50_51);
            ...
            let (bx, by, bw, bh) = find_button_in_semantics(&robot, "Jump 95")
                .expect("Jump 95 button should exist");
            robot.click(bx + bw / 2.0, by + bh / 2.0).ok();
            std::thread::sleep(Duration::from_millis(500));
            let _ = robot.wait_for_idle();
            ...
            Column(Modifier::default(), ColumnSpec::default(), move || {
                ...
                Row(
                    ...
                    move || {
                        Button(
                            Modifier::default(),
                            move || { state.scroll_to_item(50, 0.0); },
                            || { Text("Jump 50", Modifier::default()); },
                        );
                        Button(
                            Modifier::default(),
                            move || { state.scroll_to_item(95, 0.0); },
                ...
                LazyColumn(
                    ...
                    |scope| {
                        scope.items(
                            ...
                            None::<fn(usize) -> u64>,
                            move |index| {
                                let color = if index % 2 == 0 { Color::rgb(128.0, 128.0, 128.0) } else { Color::WHITE };
                                Box(
                                    Modifier::default()

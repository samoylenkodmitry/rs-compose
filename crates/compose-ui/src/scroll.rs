...
pub(crate) struct ScrollStateInner {
    ...
    /// Drag state that persists across handler recreation
    /// Stores the down position when pointer is pressed
    drag_down_position: RefCell<Option<Point>>,
    /// Last pointer position during drag
    drag_last_position: RefCell<Option<Point>>,
    /// Whether we're currently dragging
    is_dragging: RefCell<bool>,
}
...
impl ScrollState {
    ...
    pub fn new(initial: f32) -> Self {
        ...
        Self {
            inner: Rc::new(ScrollStateInner {
                ...
                drag_down_position: RefCell::new(None),
                drag_last_position: RefCell::new(None),
                is_dragging: RefCell::new(false),
            }),
        }
    }
    ...
    /// Drag state accessors (internal use by scroll modifier)
    pub(crate) fn drag_down_position(&self) -> Option<Point> {
        *self.inner.drag_down_position.borrow()
    }

    pub(crate) fn set_drag_down_position(&self, pos: Option<Point>) {
        *self.inner.drag_down_position.borrow_mut() = pos;
    }

    pub(crate) fn drag_last_position(&self) -> Option<Point> {
        *self.inner.drag_last_position.borrow()
    }

    pub(crate) fn set_drag_last_position(&self, pos: Option<Point>) {
        *self.inner.drag_last_position.borrow_mut() = pos;
    }

    pub(crate) fn is_dragging(&self) -> bool {
        *self.inner.is_dragging.borrow()
    }

    pub(crate) fn set_is_dragging(&self, dragging: bool) {
        *self.inner.is_dragging.borrow_mut() = dragging;
    }
}
